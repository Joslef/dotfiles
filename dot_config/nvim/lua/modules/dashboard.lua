	...
	if system_type ~= "darwin" then
		local output = utils.term_cmd([[
			for d in /sys/class/power_supply/BAT* /sys/class/power_supply/battery*; do
				[ -f "$d/capacity" ] && cat "$d/capacity" && exit 0
			done
			echo 0
		]])
		return tonumber(output and output:match("%d+")) or 0
	end
	local output = utils.term_cmd("pmset -g batt | grep -Eo '[0-9]+%' | head -1 | tr -d '%'")
	return tonumber(output and output:match("%d+")) or 0
end
...
local function battery_status()
	...
	if system_type ~= "darwin" then
		local status = utils.term_cmd([[
			for d in /sys/class/power_supply/BAT* /sys/class/power_supply/battery*; do
				[ -f "$d/status" ] && cat "$d/status" && exit 0
			done
			echo "Unknown"
		]])
		if not status then
		...
		return status:match("Charging") or false
	else
		local status = utils.term_cmd("pmset -g batt")
		if not status then
			return false
		end
		return status:match(" charging") or status:match("AC Power") or false
	end
end
