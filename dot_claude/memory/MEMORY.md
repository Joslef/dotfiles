...
| `notebooklm` | Use the `notebooklm` CLI (teng-lin/notebooklm-py). Steps: 1) run `notebooklm auth check --test` to verify login — if auth fails, prompt Joerg to run `notebooklm login` (opens browser); 2) run `notebooklm metadata --json` to inspect the active notebook and its sources; 3) ask Joerg what they want to do, then execute the appropriate command. Key commands: `notebooklm use <id>` (switch notebook), `notebooklm source add <url\|file>` (add source), `notebooklm ask "..."` (chat/query), `notebooklm generate audio\|video\|quiz\|flashcards\|slide-deck\|mind-map\|data-table` (generate content), `notebooklm download <type> <path>` (save artifacts). Use `--wait` on generate commands so they block until ready. **⚡ Source research — always prefer `notebooklm source add-research "<query>" --mode deep --no-wait` over doing manual web searches yourself.** This offloads the searching entirely to NotebookLM's own Google-powered research engine — zero token cost on our side, richer results, and the sources land directly in the notebook. Kick it off with `--no-wait` so it runs in the background while other work (like adding direct URLs) continues in parallel; then call `notebooklm research wait` before generating artifacts. Use `--import-all` when you want every found result auto-added without prompting. Fall back to manual `source add <url>` only for specific URLs you already know. |
