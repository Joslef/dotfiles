    ...
    Variants {
        ...
        PanelWindow {
            ...
            HyprlandFocusGrab {
                ...
                active: false
                onCleared: () => {
                    if (!active && root.monitorIsFocused)
            ...
            Connections {
                ...
                function onOverviewOpenChanged() {
                    if (GlobalStates.overviewOpen) {
                        GlobalStates.selectedWindowAddress = "";
            ...
            Connections {
                ...
                function onMonitorIsFocusedChanged() {
                    if (root.monitorIsFocused && GlobalStates.overviewOpen) {
                        delayedGrabTimer.start();
                    } else {
                        grab.active = false;
                    }
            ...
            Item {
                ...
                Keys.onPressed: event => {
                    ...
                    if (event.key === Qt.Key_Tab) {
                        ...
                        const cur = GlobalStates.selectedWindowAddress;
                        const idx = wins.findIndex(w => w.address === cur);
                        const next = event.modifiers & Qt.ShiftModifier
                            ? (idx <= 0 ? wins.length - 1 : idx - 1)
                            : (idx + 1) % wins.length;
                        GlobalStates.selectedWindowAddress = wins[next].address;
                        event.accepted = true;
                        return;
                    }
