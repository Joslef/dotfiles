...
**Universal spell rule — always do this first, before any action:**
Output the casting announcement: `** ✨🔮 CASTING SPELL <SPELLNAME> 🔮✨ **` (replace `<SPELLNAME>` with the actual spell name in uppercase). Then proceed with the spell steps.
...
- Native (non-Proton) games also supported: add Custom Game entry with direct save path (no prefix, no launch option); documented in README under "Native games" section
...
| `photoimport` | Linux | Sort photos from `/mnt/vault/Bilder/fotoimport` into `yyyy/mm/dd` hierarchy using EXIF dates; skips duplicates by MD5 hash; spell invokes with `--run`. Flags: `--fix-rotation [PATH]` fixes MOV rotation + HEIC mirror orientation. **Architecture**: fixes source files in fotoimport BEFORE copying to library (Immich never sees unfixed files). MOV logic: Orientation 5/7 → **ffmpeg** `transpose=0 -noautorotate` (selfie diagonal flip matrix `0 1 0 1 0 0 0 0 1`; uses `shutil.move` not `os.replace` to handle tmpfs→NAS cross-device); QT Rotation=0 → use pixel dimensions (H>W=90°, W>H=0°); non-zero non-mirror → use QT Rotation. HEIC fix: **`magick -auto-orient` is a complete no-op on HEIC files** — it re-encodes with lossy compression but never applies the pixel transform. Must use explicit per-orientation ops: `-flop` (orient 2 mirror H), `-flip` (orient 4 mirror V), `-transpose` (orient 5), `-transverse` (orient 7). NRT MOVs skipped (no `VideoFrameRate`). Uses `os.walk`+batched exiftool (not `-r`). Single exiftool pass fetches dates+orientation. Deps: exiftool, python3, magick, ffmpeg. Immich "delete from library" also deletes files on disk. |
