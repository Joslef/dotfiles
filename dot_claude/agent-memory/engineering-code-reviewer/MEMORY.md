...
## macfresh (macOS maintenance script)
- Reviewed 2026-03-08. See `macfresh.md` for full findings.
- Key bugs: `read` inside `confirm()` missing `|| true` (Ctrl+D aborts); `mas outdated` exits 1 when no updates exist (aborts under set -e); unquoted globs in `rm -rf ~/Library/Caches/*`.
